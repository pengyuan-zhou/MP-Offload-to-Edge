...
void load_params() {
    ...
    net = load_network("cfg/yolov3-tiny.cfg", "yolov3-tiny.weights", 0);
}
//last change
...
struct result* detect() {
    ...
    set_batch_network(net, 1);
    //srand(2222222);
    image im = load_image_color("received.jpg",0,0);
    srand(time(0));
    //char picname[20];
    //sprintf(picname, "%d_received.jpg", frmID);
    //image im = load_image_color(picname,0,0);
    image sized = letterbox_image(im, net->w, net->h);
    layer l = net->layers[net->n-1];
    double time;
    float nms=.45;
    ...
    //draw_detections(im, dets, nboxes, .5, names, alphabet, l.classes);
    free_detections(dets, nboxes);
